...
def generate_inventory_movement(num_movements):
    ...
    for _ in range(num_movements):
        for product_code in product_codes:
            ...
            if (
                (current_date.day == 1 and current_date.isoweekday() < 6)
                or (current_date.day == 2 and current_date.isoweekday() == 1)
                or (current_date.day == 3 and current_date.isoweekday() == 1)
            ):
                suppy_coefficient = product_dict.get(product_code)[1]
                received_quantity = int(
                    average_demand * 20 * random.uniform(0.7, 1.2) * suppy_coefficient
                )
            ...
            if current_date.weekday() < 5:
                issued_quantity = min(
                    int(
                        random.randint(
                            int(average_demand * 0.5), int(average_demand * 1.5)
                        )
                        * month_coefficient.get(current_date.month, 1)
                    ),
                    quantity,
                )
...
sample_inventory_movement = generate_inventory_movement(
    365
)  # Generate 31 days of movement data for March 2023
